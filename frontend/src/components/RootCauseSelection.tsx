...
interface RootCauseSelectionProps {
  options: string[];
...
export function RootCauseSelection({
  options,
  selectedOptions,
  customOption,
  onOptionSelect,
  ...
  const handleOptionClick = (option: string) => {
    // Only allow selection if under limit or if deselecting
  ...
  return (
    <div className="root-cause-selection">
      <div className="selection-header">
        <h3>
          Choose the root cause that resonates most.
          <span className="subtitle">
            (Select up to {maxSelections} if multiple feel true — {selectedOptions.length}/{maxSelections} selected)
          </span>
        </h3>
      </div>

      {/* Show selected options at the top when there are selections AND user has generated more options */}
      {selectedOptions.length > 0 && hasGenerated && (
      ...
      {/* Only show options when not generating new ones */}
      {!isGenerating && (
      ...
      <div className="generate-more-container">
        {/* Show generation limit info only when there's a message */}
        ...
        >
          {isGenerating ? (
            ...
            `Generate More Causes (${maxGenerations - generationCount} remaining)`
          )}
        </button>
      </div>
