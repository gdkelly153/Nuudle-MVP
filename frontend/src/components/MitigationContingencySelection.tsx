...
interface MitigationContingencySelectionProps {
  type: 'mitigation' | 'contingency';
  options: string[];
...
export function MitigationContingencySelection({
  type,
  options,
  selectedOptions,
  customOption,
  ...
  generationCount = 0,
  maxGenerations = 3,
}: MitigationContingencySelectionProps) {
  ...
  const getPlaceholderText = () => {
    if (type === 'mitigation') {
      return "None of the above, I'll stick with my own mitigation plan";
    } else {
      return "None of the above, I'll stick with my own contingency plan";
    }
  };

  const getGenerateButtonText = () => {
    const baseText = type === 'mitigation' ? 'Mitigations' : 'Contingencies';
    
    if (isGenerating) {
      return `Generating More ${baseText}...`;
    } else if (isGenerationLimitReached) {
      return `Generation limit reached (${maxGenerations}/${maxGenerations})`;
    } else if (isSelectionLimitReached) {
      return 'Remove a selection to generate more';
    } else {
      return `Generate More ${baseText} (${maxGenerations - generationCount} remaining)`;
    }
  };

  const title = type === 'mitigation'
    ? "Choose the mitigation that feels most effective."
    : "Choose the contingency plan that feels most practical.";
    ...
    <div className="root-cause-selection">
      <div className="selection-header">
        <h3>
          ...
          <span className="subtitle">
            (Select up to {maxSelections} if multiple feel right. If you prefer to stick with your own plan, just scroll down and click continue. {selectedOptions.length}/{maxSelections} selected)
          </span>
        </h3>
      </div>
      ...
      {selectedOptions.length > 0 && generationCount > 0 && (
        <div className="selected-options-section">
          <h4>Your Selection{selectedOptions.length > 1 ? 's' : ''}:</h4>
